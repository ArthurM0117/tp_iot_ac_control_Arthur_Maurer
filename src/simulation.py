...
ENDPOINT = "ajwzbg01qonqa-ats.iot.eu-west-1.amazonaws.com"
PATH_TO_CERTIFICATE = "files/test.cert.pem"
PATH_TO_PRIVATE_KEY = "files/test.private.key"
PATH_TO_AMAZON_ROOT_CA_1 = "terraform/files/root-CA.crt"
...
class Sensor:
    ...
    def calculate_temperature_and_add_artefact(self, is_ac_enabled: bool, zone_id: int) -> float:
        ...
        # Nouvelle logique pour le changement d'état du climatiseur
        if self.temperature > 35:  # Si la température dépasse 35 °C
            self.is_close_to_ac = True  # Activer le climatiseur
        else:
            self.is_close_to_ac = False  # Désactiver le climatiseur

        # Autres conditions de température
        if is_ac_enabled:
            temp = self.truncate(max(self.temperature * 0.988, 20))
            self.set_temperature(temp)
            self.print_temperature(zone_id)
            return temp
        else:
...
class Zone:
    ...
    def on_message_received(self, topic, payload, dup, qos, retain, **kwargs) -> None:
        new_ac_state = json.loads(payload)["state"]["enabled"]
        print("================ ZONE {} : AC SWITCH TO {} ================".format(str(self.zone_id), str(new_ac_state)))
        if new_ac_state:
            print("ZONE {} : AC SWITCH TO True".format(str(self.zone_id)))
        self.is_ac_enabled = new_ac_state
